...
export default function Student() {
  return (
    <div>
      ...
      {/* Service Cards Section with Background Color */}
      <div className="bg-blue-100 py-8"> {/* Change 'bg-gray-100' to your desired background color */}
        <div className="mx-auto container">
          <div className="grid lg:grid-cols-3 items-stretch gap-4">
            {studentServicesData.map(service => (
              <div
                key={service.id}
                className={`flex flex-col justify-between w-full p-4 group hover:shadow-md hover:shadow-black/25 transition-all ${service.bgColor} ${service.hoverColor} rounded-lg animate-slide-up`}
              >
                <div className="text-center flex-grow animate-scale-up">
                  {service.icon}
                </div>
                <h4 className="text-blue-600 text-center font-semibold text-xl my-3 underline animate-fade-in">
                  {service.title}
                </h4>
                <p className="text-gray text-sm animate-fade-in flex-grow">
                  {service.description}
                </p>
